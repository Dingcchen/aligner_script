...
def BalanceWetAlignNanocube(StepName, SequenceObj, TestMetrics, TestResults):
    ...
    while retries < 5 and not SequenceObj.Halt:
        ...
        LogHelper.Log(SequenceObj.ProcessSequenceName, LogEventSeverity.Alert, 'Nanocube top channel peak position (um) |{0:.2f}| {1:.2f}| {2:.2f}| Peak signal (V) |{2:.2f}'.format(topchanpos[0],topchanpos[1],topchanpos[2]),top_chan_peak_V)
        ...
        LogHelper.Log(SequenceObj.ProcessSequenceName, LogEventSeverity.Alert, 'Nanocube bottom channel peak position (um) |{0:.2f}| {1:.2f}| {2:.2f}| Peak signal (V) |{2:.2f}'.format(bottomchanpos[0],bottomchanpos[1],bottomchanpos[2]),bottom_chan_peak_V)
...
def BalanceWetAlignNanocubeNoPitch(StepName, SequenceObj, TestMetrics, TestResults):

    # turn on the cameras
    HardwareFactory.Instance.GetHardwareByName('DownCamera').Live(True)
    HardwareFactory.Instance.GetHardwareByName('SideCamera').Live(True)
    
    #init_V = HardwareFactory.Instance.GetHardwareByName('Hexapod').GetAxesPositions()[4] # get starting pitch value
    init_V = TestResults.RetrieveTestResult('Dry_Align_Hexapod_V')
    ...
    while retries < 5 and not SequenceObj.Halt:
        ...
        # remember the final position
        topchanpos = HardwareFactory.Instance.GetHardwareByName('Nanocube').GetAxesPositions()
        LogHelper.Log(SequenceObj.ProcessSequenceName, LogEventSeverity.Alert, 'Top channel peak position ({0:.2f}, {1:.2f}, {2:.2f}) um'.format(topchanpos[0],topchanpos[1],topchanpos[2]))
