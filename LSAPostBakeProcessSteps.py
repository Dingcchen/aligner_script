...
def SelectDie(StepName, SequenceObj, TestMetrics, TestResults):
    ...
    TestResults.AddTestResult('Post_Thermal_Cure_Measure_Start_Time', DateTime.Now);
    TestResults.AddTestResult('Post_Thermal_Cure_Measure_Operator', UserManager.CurrentUser.Name);
...
def ProbeAndMeasure(StepName, SequenceObj, TestMetrics, TestResults):
    ...
    TestResults.AddTestResult('Channel_1A_Post_Thermal_Cure_LDD_Current_mA', LDDCurr)
    TestResults.AddTestResult('Channel_2B_Post_Thermal_Cure_LDD_Current_mA', LDDCurr)

    #measure and record
    TestResults.AddTestResult('Channel_1A_Post_Thermal_Cure_Power', round(HardwareFactory.Instance.GetHardwareByName('ChannelsAnalogSignal').ReadValue('Channel1AMonitorSignal', 5), 6))
    TestResults.AddTestResult('Channel_2B_Post_Thermal_Cure_Power', round(HardwareFactory.Instance.GetHardwareByName('ChannelsAnalogSignal').ReadValue('Channel2BMonitorSignal', 5), 6))
...
def Unload(StepName, SequenceObj, TestMetrics, TestResults):
    ...
    TestResults.AddTestResult('Post_Thermal_Cure_Measure_End_Time', DateTime.Now)
